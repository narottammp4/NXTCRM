...
export function reviewEnquiries(headers: string[], rows: string[][], mapping: string[], fields: Field[], existing: any[], context: {clientId: string;projectId: string;campaignId: string;projects: any[]}, choices: Record<number, 'keep'|'discard'>) {
 ...
 const entries=all.map((lead,index)=>({lead,index,duplicates:all.flatMap((other,j)=>j!==index && enquiryKey(other)===enquiryKey(lead)?[j]:[]),existing:existing.filter(other=>enquiryKey(other)===enquiryKey(lead)),related:existing.filter(other=>phoneKey(other.phone)===phoneKey(lead.phone) && enquiryKey(other)!==enquiryKey(lead))}));
 const unresolved=entries.filter(e=>choices[e.index]!=='discard' && (e.existing.length>0 || (e.duplicates.length>0 && choices[e.index]!=='keep')));
 const keptIndexes=entries.filter(e=>choices[e.index]!=='discard' && !e.existing.length && (!e.duplicates.length || choices[e.index]==='keep')).map(e=>e.index);
